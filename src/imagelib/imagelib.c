...
Image *img_png_read_from_file(char *filename)
{
  ...
  for (int row = 0; row < height; row++)
  {
    ...
    for (int col = 0; col < width; col++)
    {
      ...
      img->pixels[row][col].R = (int)px[0];
      img->pixels[row][col].G = (int)px[1];
      img->pixels[row][col].B = (int)px[2];
    }
...
void img_png_write_to_file(Image *img, char *filename)
{
  ...
  for (int row = 0; row < img->height; row++)
  {
    ...
    {
      uint8_t *px = &(row_pointers[row][col * 4]);
      px[0] = (uint8_t)img->pixels[row][col].R;
      px[1] = (uint8_t)img->pixels[row][col].G;
      px[2] = (uint8_t)img->pixels[row][col].B;
      px[3] = 255;
    }
  }
...
/** Libera los recursos asociados a la lectura y escritura de una imagen */
